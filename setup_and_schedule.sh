...
# 顯示歡迎信息
echo -e "${BLUE}========================================${NC}"
echo -e "${CYAN}    ✨ Telegram Reviewer 自動設置工具 ✨    ${NC}"
echo -e "${BLUE}========================================${NC}"
echo
...
# 提示用戶輸入分析參數
echo
echo -e "${BLUE}⚙️ 設定 Telegram 分析參數：${NC}"
...
read -p "是否儲存分析結果為 JSON 檔案？(yes/no，預設: yes): " SAVE_RESULT
...
if [[ "$SCHEDULE" == "yes" ]]; then
    ...
    if grep -qF "$CMD" "$TEMP_CRON"; then
        echo -e "${YELLOW}⚠️ 已存在相同的定時任務，跳過添加。${NC}"
    else
...
echo
echo -e "${YELLOW}常用參數：${NC}"
echo -e "  --days <天數>      : 要分析的最近天數"
echo -e "  --top <數量>       : 熱門訊息數量"
echo -e "  --save            : 儲存分析結果為 JSON"
echo -e "  --use-history yes : 使用上次選擇的群組"
echo -e "${BLUE}========================================${NC}"
